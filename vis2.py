            ...
            if new_state != self.old_state:
                ...
                if True:
                    ...
                    if input_info.roughness!=None:
                        ...
                        roughness=torch.full((1,1,512,512),input_info.roughness[0,0,0,0],device="cuda")
...
def main(model,model2=None,datalo=None):
    ...
    dataset_paths = "/home/yley/NeuMIPforlinux/datasets/"+str(datalo)
    print(dataset_paths)
    ...
    reselution=f2c.shape[2]
    print("reselution",reselution)
    asize = f2c.shape
    asize=asize[0]
    brdf_model = None
    ...
    class MainApplication(tk.Frame):
        ...
        def draw_new_view(self,idx=None ):
            ...
            new_state.light_dir = f2l[int(new_state.idx), int(new_state.location[0]*reselution), int(new_state.location[1]*reselution)]
            new_state.camera_dir = f2camera[int(new_state.idx),int(new_state.location[0]*reselution),int(new_state.location[1]*reselution)]
            ...
            if new_state.raw_buffer != -1:
                ...
                new_view, probability_view= brdf_viewer.eval(new_state, self.brightness_b.get(), self.brightness_pg_b)
            new_view2=f2c[int(new_state.idx),:]
            #new_view2 = pow(new_view2, 1. / 2.2)
            #new_view2 = pow(new_view2, 1. / 2.2)
            # if reselution!=512:
            #     new_view=cv2.resize(new_view, (512, 512))
            #     new_view2 = cv2.resize(new_view2, (512, 512))
            global asd

            if asd!=new_state.location[0]:
                #print("camera_dir",new_state.camera_dir)
                #print("light_dir",new_state.light_dir)
                #print("location",new_state.location)
                #print("new_view",new_view[int(new_state.location[0]*reselution), int(new_state.location[1]*reselution)])
                #print("target",new_view2[int(new_state.location[0] * reselution), int(new_state.location[1] * reselution)])
                asd=new_state.location[0]
            self.viewer.set_data(new_view)
            self.viewer2.set_data(new_view2)

            self.viewer_pg.set_data(probability_view)

            # new_view_pg = brdf_viewer_pg.eval(new_state, self.brightness
            #
            # _b.get())
        ...
        def __init__(self, parent, *args, **kwargs):
            ...
            tk.Label(self.left, text='idx').grid()
            self.idx = tk.Scale(self.left, from_=0, to=asize - 1, orient=tk.HORIZONTAL,
                                        length=500, resolution=1, command=self.draw_new_view)
            self.idx.grid()
