...
class BRDFviewer:
    def __init__(self, model,model2, use_pathguiding):
    ...
    def eval(self, new_state, brightness, brightness_pg, idx=None):
        ...
        if True:
            ...
            if mimpap_type == 2:
                ...
                do_blur = True
            if roughness!=None:
                roughness=torch.full((1,1,512,512),roughness,device="cuda")
            #print(roughness)
            #print(location)
            sigma=0.5/512
            sigma = torch.full((1, 1, 512, 512), sigma, device="cuda")
            totallookupvalue=torch.cat([location,sigma,input],-3)
            #np.save("/mnt/D/querybinery/victorian"+idx+".npy", a)
            if self.model.live.args.rpe or self.model.live.args.r:
                result, eval_output = self.model.evaluate(input, location,rough=roughness, level_id=mipmap_level_id, mimpap_type=mimpap_type,
                                     camera_dir=list(camera_dir))
            else:
                result, eval_output = self.model.evaluate(input, location, rough=None,
            ...
            if self.model2!=None:
                if self.model2.live.args.rpe or self.model2.live.args.r:
                    result2,eval_output2=self.model2.evaluate(input, location,rough=roughness, level_id=mipmap_level_id, mimpap_type=mimpap_type,
                                     camera_dir=list(camera_dir),pe=False)
                else:
        ...
        return result, prob_result,result2,totallookupvalue
...
def main(model,model2=None):


    brdf_model = None
    brdf_viewer = BRDFviewer(model,model2, use_pathguiding=False)
    brdf_viewer2 = BRDFviewer(model, model2,use_pathguiding=False)
    name=str(model.live.args.inm)
    print(name,"name")
    newpath=os.path.join(path_config.path_videos , str(name) ,"cam")
    newpath2 = os.path.join(path_config.path_videos, str(name), "light")
    print(newpath)
    print(newpath2)
    ...
    class MainApplication(tk.Frame):
        ...
        def draw_new_view(self, event=None):
            ...
            y = b + r * np.sin(theta)
            print(len(x))
            oupurqueryvalue = torch.tensor([], device="cuda")
            for idx in range(len(x)):
                xcal=x[idx]
                ycal=y[idx]

                new_state.light_dir = [xcal,ycal]
                ...
                new_state.roughness = self.roughness_b.get()
                print(new_state.light_dir,new_state.camera_dir)

                new_view, probability_view,new_view2,queryvalue = brdf_viewer.eval(new_state, self.brightness_b.get(), self.brightness_pg_b.get(),idx)
                oupurqueryvalue=torch.cat([oupurqueryvalue,queryvalue],dim=0)

                #self.viewer.set_data(new_view)
                print(new_view.shape)
                new_view=new_view[ :,:,[2, 1,0]]
                print(path_config.path_videos+"/"+str(name)+"/light/"+str(idx)+".exr")


                cv2.imwrite(path_config.path_videos+"/"+str(name)+"/light/"+str(idx)+".png", new_view*255)


                self.viewer_pg.set_data(probability_view)
                if idx == len(x) - 1:
                    oupurqueryvalue = oupurqueryvalue.data.cpu().numpy()
                    np.save(path_config.path_videos+"/"+"lightchange.npy", oupurqueryvalue)
                    print(oupurqueryvalue.shape)
            ...
            for idx in range(len(x)):
                ...
                new_state.camera_dir = [xcal, ycal]
                new_state.location = self.location_selector.selected
                new_state.raw_buffer = self.raw_buffer.get()
                new_state.mipmap_level_id = self.patch_level.get()
                new_state.mimpap_type = self.mimpap_type.get()
                ...
                new_view, probability_view, new_view2,queryvalue = brdf_viewer.eval(new_state, self.brightness_b.get(),
                                                                             self.brightness_pg_b.get(),len(x)+idx)
                oupurqueryvalue=torch.cat([oupurqueryvalue, queryvalue], dim=0)

                # self.viewer.set_data(new_view)
                # print(path_config.path_videos+"/"+str(name)+"/"+str(idx)+".exr")
                new_view=new_view[ :,:,[2, 1,0]]
                print(path_config.path_videos + "/" + str(name) + "/cam/" + str(idx) + ".exr")
                #print(new_view)
                #plt.imshow(new_view)
                #plt.show
                ...
                cv2.imwrite(path_config.path_videos + "/" + str(name) + "/cam/" + str(idx) + ".png", new_view*255)
                ...
                if idx == len(x) - 1:
                    oupurqueryvalue = oupurqueryvalue.data.cpu().numpy()
                    np.save(path_config.path_videos + "/" + "camerachange.npy", oupurqueryvalue)
                    print(oupurqueryvalue.shape)
                    sys.exit()
        ...
        def __init__(self, parent, *args, **kwargs):
            ...
            self.roughness = 1
            tk.Label(self.left, text='roughness').grid()
            self.roughness_b = tk.Scale(self.left, from_=0, to=1, orient=tk.HORIZONTAL, length=500, resolution=.01,command=self.draw_new_view)
            self.roughness_b.grid()
            self.roughness_b.set(.5)
            ...
            self.raw_buffer = tk.Scale(self.left, from_=-1, to=-1, orient=tk.HORIZONTAL, length=500, resolution=1, command=self.draw_new_view)
            self.raw_buffer.grid()

            tk.Label(self.left, text='Patch level').grid()
            self.patch_level = tk.Scale(self.left, from_=0, to=0, orient=tk.HORIZONTAL, length=500, resolution=.01, command=self.draw_new_view)
            ...
            self.viewer2 = Viewer(self.right, patch_size * zoom, zoom)
            self.viewer2.grid()
