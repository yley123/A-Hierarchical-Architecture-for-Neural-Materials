...
import torch
import rectifyforbigsize230107
generate=0
path = "D:/dataset/"
dataset = h5py.File(path+rectifyforbigsize230107.dataetname+".hdf5", 'w')
print("dasdasd",path+rectifyforbigsize230107.dataetname+".hdf5")
numdata=len(rectifyforbigsize230107.files)
print(numdata)
radiuslevel=5
pathimg = rectifyforbigsize230107.pathofsave
numdatas=0
yigeshu=20
alpha=0
posionchange=1
...
def normalizedt(t):
    print("0",t.shape)
    ttotal = np.sqrt(t[:,:,0] * t[:,:,0] + t[:,:,1] * t[:,:,1] + t[:,:,2] * t[:,:,2])
    print("ttotal.shape",ttotal.shape)
    print("taverage.shape",(t[:,:,0] / ttotal).shape)
    t = torch.stack([t[:,:,0] / ttotal, t[:,:,1] / ttotal, t[:,:,2] / ttotal],dim=-1)
    print("1",t.shape)
    return(t)
...
for file in files:
    if not os.path.isdir(file): #judge wether it is a folder
        ...
        ######
        x=torch.full((512,512,1), x)
        y = torch.full((512, 512,1), y)
        z = torch.full((512, 512,1), z)
        c1=torch.full((512,512,1), -e[0])
        c2=torch.full((512,512,1), e[2])
        c3 = torch.full((512, 512,1), -e[1])
        x=coord[:,:,0].unsqueeze(dim=2)*5+x
        y=coord[:,:,1].unsqueeze(dim=2)*5+y
        print(coord[:,:,0].shape,x.shape,y.shape,z.shape)

        c1 = coord[:,:,0].unsqueeze(dim=2) * 5 + c1
        c2= coord[:,:,1].unsqueeze(dim=2) * 5 + c2


        lightdir=torch.cat([x,y,z],-1)
        print("lightdir.shape",lightdir.shape)
        camera_dir=torch.cat([c1,c2,c3],-1)



        #camera_dir = (-e[0],e[2],-e[1])
        light_dir = normalizedt(lightdir)
        #print("lightdir.shape",light_dir)
        print("testnormail",light_dir[1,1,0]*light_dir[1,1,0]+light_dir[1,1,1]*light_dir[1,1,1]+light_dir[1,1,2]*light_dir[1,1,2])
        camera_dir=(normalizedt(camera_dir))
        #print(light_dir,camera_dir)
        ...
        dircamera=camera_dir#chagestrtovector(keywordlist[1])
        #print("dircam", dircamera[0:3])
        #print(([dircamera[0], dircamera[1]]).shape)
        ground_camera_dir[idx,:]=(dircamera[:,:,0:2])
        dirlight=light_dir#chagestrtovector(keywordlist[2])
        ground_light[idx,:]=(dirlight[:,:,0:2])
