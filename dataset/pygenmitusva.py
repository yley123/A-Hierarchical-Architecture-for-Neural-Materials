...
azim=[random.uniform(-180,180) for _ in range(sample_number)]
incl=[random.uniform(0,90) for _ in range(sample_number)]
raidencrange=(0.2,0.4,0.8,1.6,3.2)
distanceset=[random.choice(raidencrange)for _ in range(sample_number)]

sample_xl=[random.uniform(-4,4) for _ in range(sample_number)]
xllimit=[]
for idxl in range(0,len(sample_xc)):
    llmm=math.sqrt(1-sample_xl[idxl]*sample_xl[idxl])
    xllimit.append(llmm)
sample_ylm=[random.uniform(-3,3) for _ in range(sample_number)]
...
for idx in range(0,sample_number):
        ...
        #render_opts.setThreads(8)  # Threads
        #render_opts.setRayBounces(64)  # ray bounces
        distanceset1=distanceset[idx]
        lux.setCameraDistance(distanceset1)
        ...
        azim1=azim[idx]
        incl1=incl[idx]
        lux.setSphericalCamera(azim1,incl1,0)
        root = lux.getSceneTree()
        for node in root.find(name="Sphere"):
                loghtnode = node
        T = luxmath.Matrix().makeIdentity().translate(luxmath.Vector((10*lightx-10*p_lightx, 10*lightx-10*p_lighty, 10*lightz-10*p_lightz)))
        p_lighty=lighty
        p_lightx=lightx
        p_lightz=lightz
        loghtnode.applyTransform(T)
        lux.renderImage((pathofsave+str(idx)+".png"), width=512, height=512)  #(pathofsave+".png"), opts=render_opts
