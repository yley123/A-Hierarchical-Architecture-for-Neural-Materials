...
class Dataset_Generator:
    ...
    def process_btf(self):
        ...
        self.res = [height, width]#get resolution
        self.dataset_res = self.res
    ...
    def process_pbrt_in(self, path, path2):
        path_query = os.path.join(path_config.path_dataset_pbrt, path)#use pbrt (a rederer)
    ...
    def generate(self):
        ...
        if self.super_smooth:
            ...
            while cur_samples < total_samples:
                ...
                base_radius = ray_buffer.base_radius#judg radius
        ...
        for idx in range(self.num_images):
            ...
            if self.dataset_in is None:
                if self.deferred:
                    deferred = True
                    scene_info = renderer.setup_test1(True, self.scene_info.resolution)#call renderer
                    ...
                    aux_buffers = (output_tensor, scene_info.zoom_in_multiplier)
                    if False:
                        aux_buffers.display()
